...
On interactive Linux SSH bootstraps, each interactive `chezmoi init --apply Markeljan` run may prompt for optional extras. All default to off, and the user can choose any combination of `desktop-vnc`, `desktop-rdp`, and `openclaw`.
...
On interactive Linux SSH bootstraps, package bootstrap can also opt into a `desktop-vnc` bundle with `Xvfb`, `x11vnc`, Openbox, a systemd-managed always-on VNC display, and Google Chrome on `amd64`, or a `desktop-rdp` bundle with XFCE desktop packages, `xrdp`, and Google Chrome on `amd64`. Re-running `chezmoi init --apply` reconciles those services: selected ones are enabled and started, deselected ones are disabled and stopped. The VNC path tries to run `x11vnc -storepasswd` when an interactive TTY is available and exports `DISPLAY=:99` through the shared shell config. The optional OpenClaw path also creates `/var/tmp/openclaw-compile-cache` when possible and exports `NODE_COMPILE_CACHE=/var/tmp/openclaw-compile-cache` plus `OPENCLAW_NO_RESPAWN=1`, including on reruns for existing installs.
